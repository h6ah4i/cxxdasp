...
bool apply_filter(const std::vector<TFrame> &src, std::vector<TFrame> &dest, const filter::filter_params_t &f_params)
{
    ...
    filter::biquad_filter<TFrame, TFilterOperator> flt;
...
#if 0
    // Direct Form 1
    typedef filter::general_biquad_direct_form_1_core_operator<app_mono_frame_t> app_fallback_mono_filter_operator_t;
    typedef filter::general_biquad_direct_form_1_core_operator<app_stereo_frame_t> app_fallback_stereo_filter_operator_t;
#if CXXPH_COMPILER_SUPPORTS_ARM_NEON
    typedef filter::f32_mono_neon_biquad_direct_form_1_core_operator app_fast_mono_filter_operator_t;
    typedef filter::f32_stereo_neon_biquad_direct_form_1_core_operator app_fast_stereo_filter_operator_t;
#else
...
#ifdef USE_ANDROID_NDK_PROFILER
#if CXXPH_COMPILER_SUPPORTS_ARM_NEON
    ::monstartup("biquad-filter");
#else
    ::monstartup("biquad-filter-no-neon");
#endif
#endif
