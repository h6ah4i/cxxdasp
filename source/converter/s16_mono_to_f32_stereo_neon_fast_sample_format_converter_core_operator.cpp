...
{
    for (int i = 0; i < n; ++i) {
        const float t = src[i] * (1.0f / 32768);
        dest[2 * i + 0] = t;
        dest[2 * i + 1] = t;
    }
}
...
void s16_mono_to_f32_stereo_neon_fast_sample_format_converter_core_operator::perform_impl(
    const int16_t *CXXPH_RESTRICT src, float *CXXPH_RESTRICT dest, int n) CXXPH_NOEXCEPT
{
