...
static inline void perform_s16_mono_to_f32_stereo_sse(const int16_t *CXXPH_RESTRICT src, float *CXXPH_RESTRICT dest,
                                                      int n) CXXPH_NOEXCEPT
{
    ...
    for (int i = 0; i < n; i += 8) {
        ...
        StoreOp::store(&dest[2 * i + 0], _mm_shuffle_ps(f32x4l, f32x4l, _MM_SHUFFLE(1, 1, 0, 0)));
        StoreOp::store(&dest[2 * i + 4], _mm_shuffle_ps(f32x4l, f32x4l, _MM_SHUFFLE(3, 3, 2, 2)));
        StoreOp::store(&dest[2 * i + 8], _mm_shuffle_ps(f32x4h, f32x4h, _MM_SHUFFLE(1, 1, 0, 0)));
        StoreOp::store(&dest[2 * i + 12], _mm_shuffle_ps(f32x4h, f32x4h, _MM_SHUFFLE(3, 3, 2, 2)));
    }
}
#endif

void s16_mono_to_f32_stereo_sse_sample_format_converter_core_operator::perform_impl(const int16_t *CXXPH_RESTRICT src,
    ...
    CXXPH_NOEXCEPT
{
    ...
    if (!utils::is_aligned(src, 16)) {
        ...
        for (int i = 0; i < n1; ++i) {
            const float t = src[i] * norm_coeff;
            dest[2 * i + 0] = t;
            dest[2 * i + 1] = t;
        }
    ...
    if (n2 > 0) {
        if (utils::is_aligned(dest, 16)) {
            perform_s16_mono_to_f32_stereo_sse<sse2_m128i_load_aligned, sse_m128_store_aligned>(src, dest, n2);
        } else {
            perform_s16_mono_to_f32_stereo_sse<sse2_m128i_load_aligned, sse_m128_store_unaligned>(src, dest, n2);
        }
