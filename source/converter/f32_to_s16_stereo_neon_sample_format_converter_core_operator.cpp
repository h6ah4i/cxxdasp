...
{
    static const float scale = (32767.0f / 32768);

    asm volatile("vldr.32 s4, [%[scale]]\n\t"
                 "1:\n\t"
                 "vldmia %[src]!, {s0}\n\t"
                 "\n\t"
                 "vmul.f32 s0, s0, s4\n\t"
...
void f32_to_s16_stereo_neon_sample_format_converter_core_operator::perform_impl(const float *CXXPH_RESTRICT src,
