...
static inline void perform_f32_to_s16_mono_sse(const float *CXXPH_RESTRICT src, int16_t *CXXPH_RESTRICT dest, int n)
    CXXPH_NOEXCEPT
{
...
void f32_to_s16_mono_sse_sample_format_converter_core_operator::perform_impl(const float *CXXPH_RESTRICT src,
    ...
    CXXPH_NOEXCEPT
{
    ...
    if (n2 > 0) {
        if (utils::is_aligned(dest, 16)) {
            perform_f32_to_s16_mono_sse<sse_m128_load_aligned, sse2_m128i_store_aligned>(src, dest, n2);
        } else {
            perform_f32_to_s16_mono_sse<sse_m128_load_aligned, sse2_m128i_store_unaligned>(src, dest, n2);
        }
