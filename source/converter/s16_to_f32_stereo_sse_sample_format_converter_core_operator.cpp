...
static inline void perform_s16_to_f32_stereo_sse(const int16_t *CXXPH_RESTRICT src, float *CXXPH_RESTRICT dest, int n)
    CXXPH_NOEXCEPT
{
...
void s16_to_f32_stereo_sse_sample_format_converter_core_operator::perform_impl(const int16_t *CXXPH_RESTRICT src,
    ...
    CXXPH_NOEXCEPT
{
    ...
    if (n2 > 0) {
        if (utils::is_aligned(dest, 16)) {
            perform_s16_to_f32_stereo_sse<sse2_m128i_load_aligned, sse_m128_store_aligned>(src, dest, n2);
        } else {
            perform_s16_to_f32_stereo_sse<sse2_m128i_load_aligned, sse_m128_store_unaligned>(src, dest, n2);
        }
