...
LOCAL_MODULE := cascaded-biquad-filter
LOCAL_SRC_FILES := cascaded-biquad-filter/cascaded-biquad-filter.cpp
...
#
# example (cascaded-biquad-filter-no-neon)
#
# if $(TARGET_ARCH_ABI) == {armeabi-v7a | armeabi-v7a-hard}
ifneq (, $(filter armeabi-v7a armeabi-v7a-hard, $(TARGET_ARCH_ABI)))
...
LOCAL_MODULE := cascaded-biquad-filter-no-neon
LOCAL_SRC_FILES := cascaded-biquad-filter/cascaded-biquad-filter.cpp
LOCAL_C_INCLUDES := $(LOCAL_PATH)/include
...
include $(CLEAR_VARS)
LOCAL_MODULE := cascaded-biquad-filter-no-neon
LOCAL_MODULE_FILENAME := cascaded-biquad-filter-no-neon-dummy
