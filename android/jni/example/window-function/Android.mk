...
LOCAL_MODULE := window-function
LOCAL_SRC_FILES := window-function/window-function.cpp
...
#
# example (window-function-no-neon)
#
# if $(TARGET_ARCH_ABI) == {armeabi-v7a | armeabi-v7a-hard}
ifneq (, $(filter armeabi-v7a armeabi-v7a-hard, $(TARGET_ARCH_ABI)))
...
LOCAL_MODULE := window-function-no-neon
LOCAL_SRC_FILES := window-function/window-function.cpp
LOCAL_C_INCLUDES := $(LOCAL_PATH)/include
...
include $(CLEAR_VARS)
LOCAL_MODULE := window-function-no-neon
LOCAL_MODULE_FILENAME := window-function-no-neon-dummy
